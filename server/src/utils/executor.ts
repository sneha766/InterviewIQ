...
const decodeB64 = (str?: string | null): string => {
  if (!str) return "";
  try {
...
export async function executeCode(
  ...
  try {
    ...
    const executionTime =
      timeInSec > 0 ? `${Math.round(timeInSec * 1000)} ms` : "< 1 ms";
    ...
    console.warn(
      "Judge0 CE execution failed or timed out, trying local host execution fallback...",
      apiError?.message
    );
...
async function executeLocalFallback(
  ...
  const tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), "iq-exec-"));
  ...
  try {
    ...
    if (language === "cpp" || language === "c++") {
      ...
      const exePath = path.join(
        tmpDir,
        os.platform() === "win32" ? "solution.exe" : "solution"
      );
      fs.writeFileSync(filePath, code);

      const compileRes = await runCmd(
        `g++ -O2 -std=c++17 "${filePath}" -o "${exePath}"`,
        tmpDir
      );
